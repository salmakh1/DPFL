...
from src.utils.attacks import FlippedLabelsDataset, random_update_attack, same_model_attack, sign_flip_model_attack
...
from src.utils.train_utils import average_weights, test, random_selection, initialize_weights, set_seed, run_wandb, \
    saving_model, testing, wandb_log, testing_
import wandb
import datetime
import logging
...
class trainClientsFL(object):
    ...
    def train_clients(self):
        ...
        if self.cfg.use_wandb:
            ...
            run_wandb("_Local", self.cfg, self.hydra_cfg, note)
        ...
        for i in range(self.cfg.num_clients):
            ...
            logging.info(f" the number of datapoints of validation are {len(val_loader.dataset)}")
            ...
            client = instantiate(self.cfg.client,
                                 ...
                                 model=copy.deepcopy(global_model),  # copy.deepcopy(global_model),
        ...
        for t in range(self.cfg.rounds):
            ...
            train_loss_batch, local_val_test_acc, local_val_test_loss, local_test_acc, local_test_loss, local_val_agg_acc, \
            local_val_agg_loss, local_test_agg_acc, local_test_agg_loss, train_acc_batch, global_local_test_acc, global_local_test_loss = [], [], [], [], [], [], [], \
                                                                                                                                          [], [], [], [], []

            # if t % 5 == 0:
            #     for client in active_clients:
            #         saving_model(client,
            #                      f'./local_models_checkpoint/client_{client.client_id}_num_client_{self.cfg.num_clients}_seed_{self.cfg.datamodule.seed}_split_{str(self.hydra_cfg["datamodule"])}_missing_{missing}_LclS_{self.cfg.client.local_steps}_sampling_{self.cfg.sampling}')
                ...
                if val_test_results["global_val_acc"] > best_val_acc[client.client_id]:
                    if self.cfg.malicious_clients:
                        if client.client_id in benign_client_ids:
                            ...
                            saving_model(client,
                                         f'./local_models/client_{client.client_id}_num_client_{self.cfg.num_clients}_seed_{self.cfg.datamodule.seed}_split_{str(self.hydra_cfg["datamodule"])}_missing_{missing}_LclS_{self.cfg.client.local_steps}_sampling_{self.cfg.sampling}')
            ...
            if self.cfg.sampling:
                ...
                wandb_log(best_val_acc_, best_test_acc_, best_agg_val_acc_, best_agg_test_acc_, train_loss_batch,
                          ...
                          local_test_agg_loss,
                          local_test_acc, local_test_loss, global_test_acc, t)
        ...
        data = [["time", "avg_time"], [total_time, total_time / self.cfg.rounds]]
        # File path
